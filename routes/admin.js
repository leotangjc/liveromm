  ...
  db.users.find({usertype: '0'}, function(err, result) {
    ...
    for (key in result) {
      if (!onlineUsers.hasOwnProperty(key._id)) {
        onlineUsers[result[key]._id] = {
          nick_name: result[key].nick_name,
...
router.get('/adminlogin', function(req, res, next) {
  db.options.find({'option_name': 'sysset'}, function(err, result) {
    ...
    res.render('room/login',
        {title: r[0].option_value[0].room_name, mes: '', scji: ''});
  });
});
  ...
  db.users.find(query, function(err, result) {
    ...
    if (result.length === 1 && result[0].level === '66') {
      ...
      var fw = {
        ...
        fwrq: new Date().toLocaleString(),
        levelname: result[0].levelname,
        pingtai: '后台登录'
      };
      console.log(fw);
      // db.fangwen.save(fw);
...
router.get('/robotlist', function(req, res, next) {
  ...
  console.log(leverlist);
  res.render('admin/robotlist',
      {username: u.nick_name, sysset: r[0], lvlist: leverlist});
});
//修改机器人
...
router.post('/dochangelesson', function(req, res, next) {
  db.options.update({option_name: 'sysset'}, {$set: {lesson: req.body.lesson}},
      function(err) {
      });
  res.end();
});
...
router.get('/chaxun', function(req, res, next) {
  res.render('admin/chaxun', {username: u.nick_name, sysset: r[0]});
});

router.get('/xianshi', function(req, res, next) {
  // req.query get localhost:3001?name=123&age=222
  // req.body  post put body
  console.log(req.query.addname);
  db.users.find({nick_name: req.query.addname}, function(err, user) {
    if (err) return res.status(400).send('internal server');
    res.send(user);
  });
  // db.name.find({name: 'parmets'});
  // res.send();
});
...
router.get('/shanchu', function(req, res, next) {
  // req.query get localhost:3001?name=123&age=222
  // req.body  post put body
  console.log(req.query.dlename);
  db.users.remove({nick_name: req.query.dlename}, function(err, user) {
    if (err) return res.status(400).send('internal server');
    res.send(user);
  });
  // db.name.find({name: 'parmets'});
...
router.get('/genxing', function(req, res, next) {
  db.users.update({nick_name: req.query.yuanming}, {
        $set: {
          nick_name: req.query.xinming,
        },
      },
      function(err, user) {
        if (err) return res.status(400).send('internal server');
...
router.get('/fangwen', function(req, res, next) {
  res.render('admin/fangwen', {username: u.nick_name, sysset: r[0]});
});

//返回访问记录
router.get('/qtfwjl', function(req, res, next) {
  console.log('当前分页' + req.query.iDisplayStart);
  db.fangwens.find({},function(err, result) {
    var fwjll = {
      'draw': 2,
      'recordsTotal': '',
      'recordsFiltered': '',
    ...
    fwjll.recordsTotal = result.length;
    fwjll.recordsFiltered = fwjll.recordsTotal;
    fwjll.data = result;
...
router.get('/messages', function(req, res, next) {
  ...
  db.messages.find({type: '0'}, function(err, result) {
    ...
    res.render('admin/messages',
        {messagevalue: result, username: u.nick_name, sysset: r[0]});
  });
});
